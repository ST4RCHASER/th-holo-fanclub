...
export class Board {
    ...
    constructor(params: { drawingSettings: any; canvasConfig: any; canvasRef: any; }) {
        // [Sketch range limits]
        const windowWidth =
            this.limitScale *
            (window.screen.width ||
        ...
        try {
            if (this.canvas) {
                // @ts-expect-error ts-migrate(2339) FIXME: Property 'once' does not exist on type 'Canvas'.
    ...
    initCanvas(canvasConfig: { zoom: number; contentJSON: null; minZoom: number; maxZoom: number; viewportTransform: number[]; }) {
    ...
    getPointerDistance(point1: { x: number; y: number }, point2: { x: number; y: number }) {
        const dx = point2.x - point1.x;
        const dy = point2.y - point1.y;
        return Math.sqrt(dx * dx + dy * dy);
    }

    addZoomListeners(params?: any) {
        ...
        canvas.on('touch:gesture', (event) => {
            ...
            const touchs = (event.e as TouchEvent)?.touches;
            if (touchs && touchs.length === 2) {
                const point1 = {
                    x: touchs[0].clientX,
                    y: touchs[0].clientY,
                };
